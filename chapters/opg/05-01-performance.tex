...
\begin{table}[ht!]
    ...
    \end{tabular}
		\end{footnotesize}
		\vspace*{-3mm}
    \caption{\label{eval:mpsc} \algoMRSimple~running time (seconds)}
		\vspace*{-4mm}
\end{table}
...
%Fig.~\ref{fig:mpsc-example}. The computation time of \algoMRSimple~under 
...
\begin{table}[ht!]
    ...
    \end{tabular}
    \vspace*{-3mm}
    \caption{\label{eval:spmc} \algoSRG~computation time (seconds)}
    \vspace*{-4mm}
\end{table}
...
%An example problem instance along with its optimal cover is shown in 
%Fig.~\ref{fig:spmc-example}. The computation time of 

%\subsection{Problem Statement for Perimeter Guarding}
...
\begin{align}\label{eq:l-star}
\vspace*{-1mm}
\ell^* = \min_{\C} \max_{C_j \in \C} len(C_j).
\vspace*{-1mm}
\end{align}

Fig.~\ref{fig:example} shows an example of an optimal cover by $8$ robots 
...
more formally in Section~\ref{section:analysis} 
(Proposition~\ref{p:max-no-exclusion}).
...
Fig.~\ref{fig:example-boundaries}, $G$ may be one of the three dotted 
...
\begin{lemma}\label{l:no-partial-coverage} 
